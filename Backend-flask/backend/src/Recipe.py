...
class Recipe:
    ...
    def create(self, recipeID, title, description, photoRecipe, serve, author, prep_time, cook_time, yield2, tag, ingredients, steps):
        self.db.execute ("INSERT INTO tbl_recipe \
                          VALUES ('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}')".format(recipeID, title, description, photoRecipe,  serve, author, prep_time, cook_time, yield2, tag, ingredients, steps))
        
        return {'Is Error': False, 'Error Message': ""}

    #add recipe 
    def addRecipe(self, title, description, photoRecipe, serve, author, prep_time, cook_time, yield2, tag, ingredients, steps):
        data, columns = self.db.fetch ("SELECT MAX(rp.recipe_id) FROM tbl_recipe rp")
        newID = increaseID(data[0][0],"rp")
        log = self.create(newID, title, description, photoRecipe, serve, author, prep_time, cook_time, yield2, tag, ingredients, steps)
        return log
    ...
    def getReceipeByID(self,id):
        ...
        data, columns = self.db.fetch ("SELECT r.recipe_id, r.title, r.description, r.serve, r.photo_recipe, r.author, usr.profile_name, r.prep_time"
                              " , r.cook_time, r.yield2 "
                              " FROM tbl_recipe r JOIN tbl_user usr ON r.author = usr.user_id WHERE r.recipe_id = '{}' ".format(id))
        return row_as_dict(data, columns)

    #update recipe 
    def updateReceipeByID(self, recipe_id, newTitle, newDescription, newPhotoRecipe, newServe):

        data, columns = self.db.fetch ("SELECT * FROM tbl_recipe WHERE recipe_id = '{}' ".format(recipe_id))

        if len(data) > 0:
        
            self.db.execute ("UPDATE tbl_recipe SET title = '{}', description = '{}', photo_recipe = '{}', serve = {} WHERE recipe_id = '{}' ".format(newTitle, newDescription, newPhotoRecipe, newServe,recipe_id))
        else:
